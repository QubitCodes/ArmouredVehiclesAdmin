...
export default function ProductAccordionForm({ productId, domain, readOnly = false }: ProductAccordionFormProps) {
    ...
    const updateLocalSpec = (index: number, field: keyof ProductSpecification, val: any) => {
        ...
        if (field === 'label' || field === 'value') {
            ...
            // Sync specific attributes to main form state for dual-column saving
            const label = updated[index].label?.trim();
            if (label === 'Condition') {
                form.setValue('condition', updated[index].value || '');
            } else if (label === 'Color') {
                const colors = (updated[index].value || '').split(',').map(s => s.trim()).filter(Boolean);
                form.setValue('colors', colors);
            } else if (label === 'Size') {
                // If there are multiple size rows, this logic might need adjustment, 
                // but usually we want at least one or a collection.
                // For now, let's collect all 'Size' values from all rows.
                const allSizes = updated
                    .filter(s => s.label?.trim() === 'Size' && s.active)
                    .map(s => s.value || '')
                    .filter(Boolean);
                form.setValue('sizes', allSizes);
            }
        }
